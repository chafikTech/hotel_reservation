...
func (h *RoomHandler) HandleBookRoom(c *fiber.Ctx) error {
	...
	ok, err = h.isRoomAvailableForBookig(c.Context(), roomOID, params)
	if err != nil {
		return err
	}
	if !ok {
		return c.Status(http.StatusBadRequest).JSON(genericResp{
			Type: "error",
			Msg:  fmt.Sprintf("room %s already booked", roomOID.String()),
		})
	}
...
func (h *RoomHandler) isRoomAvailableForBookig(ctx context.Context, roomID primitive.ObjectID, params BookRoomParams) (bool, error) {
	where := bson.M{
		"roomID": roomID,
		"fromDate": bson.M{
			"$gte": params.FromDate,
